...
void Knight::getAllPossibleMoves(std::pair<char, int> currentPosition) {
    ...
    std::pair<char, int> newMove = std::make_pair(currentPosition.first + 1, currentPosition.second + 2);
    if (this->isMoveInBounds(newMove)) {
        this->allPossibleMoves.push_back(newMove);
    }

    // move 2 up and 1 left
    newMove = std::make_pair(currentPosition.first - 1, currentPosition.second + 2);
    if (this->isMoveInBounds(newMove)) {
        this->allPossibleMoves.push_back(newMove);
    }
    ...
    newMove = std::make_pair(currentPosition.first + 2, currentPosition.second + 1);
    if (this->isMoveInBounds(newMove)) {
        this->allPossibleMoves.push_back(newMove);
    }
