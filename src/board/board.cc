...
void Board::movePiece(std::pair<char, int> from,
...
void Board::movePieceBase(std::pair<char, int> from, std::pair<char, int> to) {
    ...
    if (fromPiece->getName() != '*' && (fromPiece->getColor() != toPiece->getColor())) {
        ...
        if (this->currentBoard[to.first - 'a'][to.second - 1]->getName() ==
            'k') {
            this->setBlackKingPosition(to);
        } else if (currentBoard[to.first - 'a'][to.second - 1]->getName() ==
                   'K') {
